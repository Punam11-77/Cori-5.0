...
""")
       ...
       """) 
        ...
        b1_up.clicked.connect(self.move_rapid_up)
        b1_down.clicked.connect(self.move_rapid_down)

        b2_up.clicked.connect(self.move_command_up)
        b2_down.clicked.connect(self.move_command_down)
    ...
    def move_rapid_up(self):

        current = self.rapid_list.currentRow()

        if current > 0:
            self.rapid_list.setCurrentRow(current - 1)

    def move_rapid_down(self):

        current = self.rapid_list.currentRow()

        if current < self.rapid_list.count() - 1:
            self.rapid_list.setCurrentRow(current + 1)
